...
Некоторые фичи:
*   Показывает ссылки релевантные тексту;
*   Подсказывает анкоры (v 1.3+);
*   Экспорт перелинковки в файл (v 1.4+);
*   Отмечает все ссылки, которые уже присутствуют в тексте;
*   Показывает, если ссылка повторяется несколько раз;
*   Считает общее количество вставленных и найденных ссылок в тексте;
*   Вставка ссылки вокруг выделенного текста в 1 клик;
*   Работает в режиме HTML и RichText;
*   Поиск и подстветка уже вставленных ссылок по клику;
*   Возможность скрыть уже присутствующие ссылки из списка;
*   Быстрый фильт для поиска нужной ссылки;
*   Работает при редактировании стандартных записей, страниц, категорий и кастомных типов;
*   При удалении плагина перелинковка не пропадет.

    **CherryLink работает только с текстовым и визуальным редакторами WP (TinyMCE). Gutenberg пока не поддерживается, но в планах. Различные интерактивные PageBuilder’ы не поддерживаются и не будут.**
...
1. Активируйте на странице плагинов в админке WP
1. Перейдите в Настройки->CherryLink для изменения опций плагина
1. Чтобы активировать лицензионный ключ, зайдите в раздел "Разное"

== Frequently Asked Questions ==

= Есть ли поддержка Gutenberg? =

Пока нет, гутенберг не позволяет полностью реализовать функционал плагина. Ждем поддержки нужных функций в будущем.

= Не находит релевантные статьи, что делать? =

Проверьте настройки фильтрации плагина, затем сделайте реиндексацию (Разное - Сохранить и реиндексировать). Если не поможет - обратитесь к разработчику.

== Screenshots ==

1. Скриншот панели перелинковки в редакторе screenshot-1.png
...
1. Инструкция по установке [Установка](https://seocherry.ru/dev/install-cherrylink/ "Если возникнут сложности")
1. Подробный мануал по использованию [Руководство](https://seocherry.ru/dev/cherrylink-manual/ "Есть ответы на многие вопросы")
1. Дополнение для вывода блока ссылок [Related Block](https://seocherry.ru/dev/cherrylink-related-block/ "Работает только с основным плагином")
