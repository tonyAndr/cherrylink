jQuery(document).ready(function($){
    ...
    $('#linkate-box').find('h2').after('<div class="linkate-filter-bar"><div><div><input id="hide_that_exists" type="checkbox"><label class="'+cl_exists_class+'" for="hide_that_exists">LINK</label></div><div><input id="show_that_exists" type="checkbox"><label for="show_that_exists" class="linkate-link">LINK</label></div></div><div><input id="filter_by_title" type="text" placeholder="Фильтр"><span class="filter-clear-box"></span></div></div><div class="linkate-tabs"><div class="tab tab-articles linkate-tab-selected">Записи</div><div class="tab tab-taxonomy">Таксономии</div></div><div class="suggestions-panel-back">&#10094; Назад</div>');
    $('#linkate-box').append('<div class="linkate-total-links"><div class="total-links-header">Статистика перелинковки</div><div class="total-links-counter"><div> Исходящих</div><div id="links-count-total">?</div><div> Входящих</div><div id="links-count-targets">?</div></div></div>');
    
    $('.container-articles').append('<div class="linkate-load-more"><div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div><div class="load-more-text">Загрузить еще...</div></div>');
    ...
    // links stats in/out
    function fcl_individual_links_stats() {
